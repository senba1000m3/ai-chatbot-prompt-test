...
export function CompareVersionCard({
	...
	return (
		<motion.div
			...
			draggable={!hideGrip}
			onDragStart={!hideGrip ? (e) => onDragStart(e, version) : undefined}
			onDragOver={!hideGrip ? (e) => onDragOver(e, index) : undefined}
			onDragLeave={!hideGrip ? onDragLeave : undefined}
			onDrop={!hideGrip ? (e) => onDrop(e, index) : undefined}
			onDragEnd={!hideGrip ? onDragEnd : undefined}
		>
			<Card
				className={`bg-gray-900 border border-gray-700 rounded-lg px-3 pt-3 pb-1 cursor-pointer transition-all duration-200 ${colorConfig.border} ${isDragOver ? "ring-2 ring-blue-500" : ""}`}
				onClick={() => onToggleExpand(version.id)}
			>
				{/* 標題與時間 */}
				<div className="flex items-center justify-between mb-3">
					<div className="flex items-center space-x-2 flex-1 min-w-0">
						{!hideGrip && <GripVertical className="w-3 h-3 text-gray-400" />}
						<h3 className="font-medium text-white truncate flex-1 text-sm">{version.name}</h3>
					</div>
					<div className="flex items-center space-x-2">
						<Badge variant="outline" className={`text-[10px] text-white border-gray-600 ${colorConfig.badge}`}>{`版本 ${versionNumber}`}</Badge>
						<motion.div animate={{ rotate: isExpanded ? 90 : 0 }} transition={{ duration: 0.2 }}>
							<ChevronRight className="w-3 h-3 text-gray-400" />
						</motion.div>
					</div>
				</div>
				<div className="text-xs text-gray-400 mb-2" style={{whiteSpace: "pre"}}>儲存時間: {version.savedAt.toLocaleString().split(".")[0].replace("T","  ")}</div>
				<AnimatePresence>
					{isExpanded && (
						<motion.div
							...
							className="space-y-2 pt-2 border-t border-gray-700 overflow-hidden"
						>
							{/* System Prompt 詳情 */}
							<div>
								<h4 className="text-xs font-medium text-white mb-1">System Prompt</h4>
							...
							<div>
								<h4 className="text-xs font-medium text-white mb-1">Default Hint Message ({safeVersionData.hintMessage.length})</h4>
								<div className="space-y-1.5">
							...
							{/* Parameters 詳情 */}
							<div>
								<h4 className="text-xs font-medium text-white mb-1">Parameters</h4>
							...
							{/* Models 詳情 */}
							<div>
								<h4 className="text-xs font-medium text-white mb-1">Models ({safeVersionData.models.length})</h4>
							...
							{Array.isArray(version.data?.tools) && (
								<div>
									<h4 className="text-xs font-medium text-white mb-1">Tools ({version.data.tools.length})</h4>
									<div className="flex flex-wrap gap-1">
